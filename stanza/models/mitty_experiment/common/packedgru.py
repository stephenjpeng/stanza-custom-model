...
class PackedGRU(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, bias=True, batch_first=False, dropout=0, bidirectional=False, pad=False, rec_dropout=0):
        ...
        self.pad = pad
        if rec_dropout == 0:
            # use the fast, native GRU implementation
            ...
            self.gru = GRUwRecDropout(input_size, hidden_size, num_layers, bias=bias, batch_first=batch_first, dropout=dropout, bidirectional=bidirectional, rec_dropout=rec_dropout)
    ...
    def forward(self, input, lengths, hx=None):
        ...
        res = self.gru(input, hx[0])
        if self.pad:
            res = (pad_packed_sequence(res[0], batch_first=self.batch_first)[0], res[1])
        return res

class GRUwRecDropout(nn.Module):
    """ An GRU implementation that supports recurrent dropout """
    def __init__(self, input_size, hidden_size, num_layers, bias=True, batch_first=False, dropout=0, bidirectional=False, pad=False, rec_dropout=0):
        super().__init__()
        self.batch_first = batch_first
        self.pad = pad
        ...
        for l in range(num_layers):
            ...
            for d in range(self.num_directions):
                self.cells.append(nn.GRUCell(in_size, hidden_size, bias=bias))
